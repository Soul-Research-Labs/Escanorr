...
fn main() {
    ...
    // Send 400 with 10 fee
    let outputs = esc.send(recipient_owner, 400, 10).expect("send failed");
    println!("Sent 400 to recipient (fee: 10)");
    println!("Recipient note: value={}", outputs[0].value);
    println!("Change note: value={}", outputs[1].value);
